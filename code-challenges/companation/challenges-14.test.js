...
const createServer = () => {
  ...
  app.delete("/things/1", (req, res) => {
    res.status(405).send("Hello Again?");
  });
...
const toTitleCase = (arr) => {
  ...
  return arr.map((str) => {
    ...
    let raminStr = str.substring(1);
    return `${firstLetter}${raminStr}`;
  });
};
...
let biggerThanLuke = (arr) => {
  ...
  let names = arr
    .filter((item) => {
      if (parseInt(item.mass) > parseInt(lukesMass[0].mass)) {
        return item;
      }
    })
    .map((charac) => {
